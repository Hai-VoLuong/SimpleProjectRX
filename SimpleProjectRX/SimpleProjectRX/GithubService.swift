...
class GithubService {
    ...
    func getLangueList(completionHandle: (Result<[String]>) -> Void) {
        let listOfPopularLanguages = ["Swift","Objective-C","Java","C","C++","Python","C#"]
        completionHandle(.success(listOfPopularLanguages))
    }
        ...
        session.dataTask(with: url) {data, reponse, error in
            DispatchQueue.main.async {
                ...
                guard let data = data,
                    ...
                    let itemJson = jsonDict["items"] as? [[String: Any]] else {
